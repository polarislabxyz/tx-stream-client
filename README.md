...
Standalone Rust client and decoder for the custom
`polaris.solana.fastpath.v1.SolanaTransactionFastpath/SubscribeResolvedTransactions`
gRPC service. Version `0.1.0-rc.1` is a release candidate. The customer endpoint
must have FastPath enabled for your API key before the example can subscribe.
...
cargo run --locked -p subscribe-resolved -- --help
cargo run --locked -p subscribe-resolved
```

Optional: `POLARIS_TX_STREAM_MAX_FRAMES=100` stops after 100 frames. Positional base58
account keys form one include-any filter. Never put your API key in source,
command-line arguments or bug reports. The client sends `x-api-key` metadata.
...
```toml
polaris-tx-stream-client = { git = "https://github.com/polarislabxyz/tx-stream-client", tag = "v0.1.0-rc.1" }
```
...
These are provisional, eagerly resolved transactions observed from shreds.
...
Match-all sequences must be contiguous. Filtered sequences may skip but must
...
The advanced `from_channel` API delegates TLS configuration to its caller.
